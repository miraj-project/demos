(ns miraj.demo.basic
  (:require [clojure.java.io :as io]
            ...
            ;; [miraj.demo.hello-world-polymer :as greeter] ; :refer :all] ; [hello]]
...
(println "loading miraj.demo.basic")

(defprotocol HelloWorldProtocol
  (foo [this])
  (bar [this x]))

(miraj/defcomponent hello-world :using hello
  ;; specify "helper" cljs ns, one of:
  ;; :as hi  ;; == miraj.demo.basic.hello :as hi
  ;; :delegate [miraj.demo.hello-impl :as hi]  ;; component will delegate to cljs fns
  ;; default delegate is same as :using name

  ;; using :delegate allows us to reserve :require for clojure libs
 ...
 span {background-color:#E0F2F1;}")
   ...
   (h/div (h/h4 "Another Fine Message!"))
   (h/div 'message)
  ...
  ;; polymer prototype
  {:polymer/properties {:greeting ^String{:value "hello" ; miraj.demo.basic.hello/msg
                                          ;; :type String
                                          ;; FIXME: alias for helper ns
                                          :observer (fn [new old] (h/observe-greeting new old))
                                          ;;:observer (clj->js 'h/observe-greeting)
                                          }
                        :addressee {:value "WORLD"
                                    ;; :type String
  ...
  (click [e] (this-as this
               ...
               (miraj.demo.basic.hello/click this e))) ;; [x] (.log js/console "Mouseclick"))
  (mouseover [x] (miraj.demo.basic.hello/mouseover x)) ; (.log js/console "MOUSEOVER"))
  )

                                        ;(-> hello var meta :miraj/miraj keys)
                                        ;(-> basic var meta)
(println "loaded miraj.demo.basic")
